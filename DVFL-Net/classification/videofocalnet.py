...
class VideoFocalNetBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, mlp_ratio=4., drop=0., drop_path=0., 
        ...
        #print(">< Dimension of input features >< : ", dim)
        #print(">< Dimension of hidden features >< : ", mlp_hidden_dim)
       
        #hidden_dim = int(dim * mlp_ratio) 
        #self.kan = KAN(in_features=dim, hidden_features=hidden_dim)
    ...
    def forward(self, x):
        ...
        #x = x + self.drop_path(self.gamma_2 * (self.norm2(self.kan(x)) if self.use_postln else self.kan(self.norm2(x)))) 
...
@register_model
def videofocalnet_nano(pretrained=False, **kwargs):
    model = VideoFocalNet(depths=[1, 1, 2, 1], embed_dim=96, **kwargs)
    if pretrained:
        url = model_urls['videofocalnet_tiny']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint["model"])
